...
router.get("/:id", validatorHandler(getQuestionChainSchema, "params"), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { id } = req.params;
...
router.post("/", validatorHandler(createQuestionChainSchema, "body"), async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { body } = req;
...
router.put("/:id", validatorHandler(getQuestionChainSchema, "params"), validatorHandler(updateQuestionChainSchema, "body"), async (req: Request, res: Response, next: NextFunction) => {
  const { body } = req;
  const { id } = req.params;
...
router.delete("/:id", validatorHandler(getQuestionChainSchema, "params"), async (req: Request, res: Response, next: NextFunction) => {
  const { id } = req.params;
  try {
